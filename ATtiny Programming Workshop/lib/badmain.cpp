...
// displayAddress must be incremented by 1 if the device is not using the first address.
int displayAddress = 0b0111100; // last bit may need to be incremented. Displays are all 0b0111100 or 0b0111101
int rw = 0b0;
int ack = 0b0;
int co = 0b0;
int dc = 0b0;
int controlByte = 0b00000000;
//int data[5];

int data[5] = {displayAddress, rw, ack, co, dc};
#define PIN0 0
#define PIN2 2

void setup() {
    Wire.begin(); // establish I2C interface
    Wire.setClock(100000); // set clock to 100kHz
    pinMode(PIN0, OUTPUT);
    pinMode(PIN2, OUTPUT);
};
...
void loop() {
    ...
    Wire.endTransmission(); //will send all data in buffer
}
*/
...
#ifndef SSD1306_SCL
#define SSD1306_SCL		PB2	// SCL,	Pin 3 on SSD1306 Board. Would be nice to see on PB2
#endif
#ifndef SSD1306_SDA
#define SSD1306_SDA		PB0	// SDA,	Pin 4 on SSD1306 Board. Would be nice to see on PB0
#endif
#ifndef SSD1306_SA
...
void loop() { 
  delay(40);
	...
	while (1==1) {
              ...
              // frame actions
              if ( (uint16_t)lastFrame + 10 < millis()){
